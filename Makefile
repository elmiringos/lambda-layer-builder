...
clean: validate-version
	@echo "Cleaning build artifacts for Python $(PYTHON_VERSION)..."
	@rm -f python-layer-$(PYTHON_VERSION).zip
	@docker rmi lambda-layer-builder-py$(PYTHON_VERSION) 2>/dev/null || true
	@echo "Clean complete."
...
clean-all:
	...
	@for version in $(VALID_VERSIONS); do \
		docker rmi lambda-layer-builder-py$$version 2>/dev/null || true; \
	done
	@echo "Clean complete."
...
test: validate-version
	@echo "Testing Lambda layer for Python $(PYTHON_VERSION)..."
	@echo ""
	@if [ ! -f "$(OUTPUT_ZIP)" ]; then \
		echo "Error: Layer file $(OUTPUT_ZIP) not found. Run 'make build' first."; \
		exit 1; \
	fi
	@echo "1. Layer file exists: $(OUTPUT_ZIP)"
	@UNZIPPED_SIZE=$$(unzip -l "$(OUTPUT_ZIP)" | tail -1 | awk '{print $$1}'); \
	...
	@ZIP_SIZE=$$(stat -f%z "$(OUTPUT_ZIP)" 2>/dev/null || stat -c%s "$(OUTPUT_ZIP)"); \
	...
	@if unzip -l "$(OUTPUT_ZIP)" | grep -q "python/"; then \
	...
	@unzip -l "$(OUTPUT_ZIP)" | grep "python/[^/]*/" | sed 's/.*python\//   /' | cut -d'/' -f1 | sort -u | head -20
	@echo ""
	@echo "All tests passed!"

# Upload layer to AWS Lambda
upload: validate-version
	@echo "Uploading Lambda layer for Python $(PYTHON_VERSION)..."
	@if [ ! -f "$(OUTPUT_ZIP)" ]; then \
		echo "Error: Layer file $(OUTPUT_ZIP) not found. Run 'make build' first."; \
		exit 1; \
	fi
	...
	@LAYER_FULL_NAME="$(LAYER_NAME)-py$$(echo $(PYTHON_VERSION) | tr -d '.')"; \
	...
	aws lambda publish-layer-version \
		...
		--description "Python $(PYTHON_VERSION) dependencies layer" \
